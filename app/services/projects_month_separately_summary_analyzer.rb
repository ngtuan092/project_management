class ProjectsMonthSeparatelySummaryAnalyzer < Patterns::Service
  ...
  def value_resources_months_total
    ...
    month_number_displayed(start_month_year, end_month_year).each do |ele|
      hash_out[ele.reverse.join "-"] =
        value_resources_month_total ele, project_ids
    end
  ...
  def value_resources_year_total projects_month
    # sum all values 12 month 1 + 2 + 3 ... of list projects
  ...
  def value_resources_month_total month_year, project_ids
    ...
    resource_total = ProjectUserResource
                     .total_man_month_projects project_ids, month_year[1],
                                               month_year[0]
    # hash :month
    {value: value_total.round(Settings.digits.length_2),
